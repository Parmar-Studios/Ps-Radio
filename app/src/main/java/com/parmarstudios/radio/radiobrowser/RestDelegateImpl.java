...
class RestDelegateImpl implements RestDelegate {
    ...
    private HttpURLConnection newClient(
        ...
        Log.d(this.getClass().getSimpleName(), "Connecting to " + url.toExternalForm());
        ...
        for (String part : parts) {
            if (part.trim().startsWith("charset=")) {
                ...
                try {
                    ...
                    Log.w(RestDelegateImpl.class.getSimpleName(), "Illegal charset name: {}" + charsetName);
                    return StandardCharsets.UTF_8;
                } catch (UnsupportedCharsetException e) {
                    Log.w(RestDelegateImpl.class.getSimpleName(), "Unsupported charset name: {}" + charsetName);
                    return StandardCharsets.UTF_8;
                }
            }
        }
    ...
    private <T> T retryLoop(final Supplier<T> supplier) {
        ...
        while (true) {
            try {
                ...
                try {
                    Log.d(this.getClass().getSimpleName(), "Got HTTP " + e.code + ", retrying in " + retryInterval + " ms");
    ...
    private void sendXWWWFormUrlencodedRequest(final HttpURLConnection connection,
        ...
        Log.d(RestDelegateImpl.class.getSimpleName(), "POST WWW-Form-UrlEncoded body: " + encoded);
    ...
    private void sendJsonRequest(final HttpURLConnection connection,
        ...
        Log.d(RestDelegateImpl.class.getSimpleName(), "POST JSON body: " + json);
        connection.setRequestProperty(
                "Content-Type",
    ...
    private static void logResponseStatus(
            final HttpURLConnection response) throws IOException {
        if (response.getResponseCode() != HttpURLConnection.HTTP_OK) {
            Log.w(RestDelegateImpl.class.getSimpleName(), "Non HTTP OK/200 status: status=" + response.getResponseCode() + ", reason=" + response.getResponseMessage());
        } else {
            Log.d(RestDelegateImpl.class.getSimpleName(), "HTTP response status=" + response.getResponseCode() + ", reason=" + response.getResponseMessage() + ", length=" + response.getContentLength());
        }
    }
}